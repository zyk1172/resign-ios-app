...
- **增量构建缓存（免重复编译）**：
  ...
  - 每次安装日志输出嵌入 profile 的**签名有效期与所属团队**（"签名有效期至: …（Team XXXX）"），续签是否刷新直接可查；
  - **签名构建前清除该 App 的旧 provisioning profile**（含扩展）：xcodebuild 会复用存储里"仍然有效"的旧 profile，即使项目已切换 Team 或设备注册变化——那正是"装上不能用"的来源；清除后强制为当前团队与设备重新生成；
