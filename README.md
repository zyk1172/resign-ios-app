...
- 项目未变化时自动复用增量构建缓存（保留 DerivedData，跳过无变化编译，仅重跑签名阶段刷新有效期）；可在项目设置中手动清除缓存；
- 后台定时任务由 App 内置的 **ResignWorker**（独立 Swift 可执行文件）执行，与手动执行共用同一构建引擎、设备发现、产物解析、错误分类和重试策略，不再维护两套实现；
...
定时任务每天在指定时间检查一次，由 `launchd` 启动 App 内的 `Resign.app/Contents/MacOS/ResignWorker --scheduled-run`。Worker 读取与 App 相同的 `config.json`，按"上次全部成功安装时间 + 间隔天数"逐项目判断是否到期，只有到期的项目才会真正构建；手动执行成功同样会推进到期时间。Mac 睡眠错过日历时间后，`launchd` 会在唤醒时处理该次计划；用户退出登录或设备不可用时无法完成安装。
...
Resign 不上传项目、设备或签名信息。Xcode 负责访问钥匙串中的开发者凭据。App 的配置（项目、设置、执行状态和日志的唯一事实源）和日志保存在：
...
- 后台任务不再生成 Bash 脚本，构建/签名/安装/产物解析/重试逻辑只存在于 Swift 执行核心（`BuildCoordinator`），LaunchAgent 仅负责触发 Worker；
- 手动执行与后台任务共用跨进程构建锁（`resign.lock`），无法并发进入构建区域；配置读写使用独立的跨进程锁（`config.lock`）做原子合并；
...
- 编译/签名类确定性错误不会长时间盲目重试；无法识别的错误同样不自动重试。
